...
function findAssetRenameOrderBatches(assetGraph, queryObj) {
    ...
    clone.findAssets({url: query.defined}).forEach(function (asset) { // FIXME: Query
        if (!assetMatcher(asset)) {
            clone.findRelations({to: asset}).forEach(function (incomingRelation) { // FIXME: Unnecessary if AssetGraph.removeAsset doesn't warn about implicit removal of incoming relations
                clone.removeRelation(incomingRelation);
            });
