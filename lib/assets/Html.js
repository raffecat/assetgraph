...
extendWithGettersAndSetters(Html.prototype, {
    ...
    alternativeExtensions: ['.template', '.xhtml', '.shtml'],
    ...
    get parseTree() {
        if (!this._parseTree) {
            try {
                ...
                var err = new Error('Parse error in ' + (this.url || 'inline Html' + (this.nonInlineAncestor ? ' in ' + this.nonInlineAncestor.url : '')) + '\n' + e.message);
                if (this.assetGraph) {
    ...
    _isRelationUrl: function (url) {
        ...
        return url && !/^javascript:|^mailto:|^\s*$|^#/i.test(url);
    },
        ...
        while (queue.length) {
            ...
            if (node.nodeType === node.ELEMENT_NODE) {
                ...
                if (nodeName === 'script') {
                    ...
                    } else if (type === 'text/html') { // html include.
                        var src = node.getAttribute('src');
                        if (src) {
                            if (this._isRelationUrl(src)) {
                                outgoingRelations.push(new relations.HtmlScript({
